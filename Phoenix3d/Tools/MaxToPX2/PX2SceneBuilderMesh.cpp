								  ...
								  std::vector<int>& faceIndexParts)
{
	...
	uniMesh->ColorMap() = new1<PX2::Float3>(uniMesh->CQuantity());
									  ...
									  std::vector<int>& faceIndexParts)
{
	...
	for (i = 0; i < (int)faceIndexParts.size(); i++)
	{
		TVFace &texFace = maxMesh->tvFace[faceIndexParts[i]];
		for (j = 0; j < 3; j++)
		{
			texIndexs.insert(texFace.t[j]);
		}
	}
	...
	std::set<int>::iterator iter = texIndexs.begin();
	for (i = 0; i < (int)texIndexs.size(); i++, iter++)
	{
		j = *iter;
		texMap[j] = i;

		(uniMesh->TextureCoordMap()[i])[0] = maxMesh->tVerts[j].x;
		(uniMesh->TextureCoordMap()[i])[1] = 1.0f - maxMesh->tVerts[j].y;
	}

	uniMesh->TFace() = new1<int>(3*uniMesh->FQuantity());
	for (i = 0; i < (int)faceIndexParts.size(); i++)
	{
		TVFace& rkTFace = maxMesh->tvFace[faceIndexParts[i]];
		for (j = 0; j < 3; j++)
		{
