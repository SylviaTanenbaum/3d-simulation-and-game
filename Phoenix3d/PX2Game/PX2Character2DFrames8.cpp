...
//----------------------------------------------------------------------------
void Character2DFrames8::SaveToXMLNode (XMLNode nodeParent)
{
	XMLNode nodeClass = nodeParent.NewChild("Class");
	nodeClass.SetAttributeString("Type", "Character2DFrames8");

	Character::SaveToXMLNode(nodeClass);

	XMLNode nodeProperty = nodeClass.NewChild("Property");
	nodeProperty.SetAttributeFloat("AnimSize", GetAnimSize());
	nodeProperty.SetAttributeFloat("Offset_X", mOffset.X());
	nodeProperty.SetAttributeFloat("Offset_Y", mOffset.Y());
	nodeProperty.SetAttributeFloat("Offset_Z", mOffset.Z());
	nodeProperty.SetAttributeFloat("RolateNodeOffset_X", mRolateNodeOffset.X());
	nodeProperty.SetAttributeFloat("RolateNodeOffset_Y", mRolateNodeOffset.Y());
	nodeProperty.SetAttributeFloat("RolateNodeOffset_Z", mRolateNodeOffset.Z());
	nodeProperty.SetAttributeFloat("AttackFrontOffset_X", mAttackFrontOffset.X());
	nodeProperty.SetAttributeFloat("AttackFrontOffset_Y", mAttackFrontOffset.Y());
	nodeProperty.SetAttributeFloat("AttackFrontOffset_Z", mAttackFrontOffset.Z());
	nodeProperty.SetAttributeString("ShadowFilename", GetShadowFilename());
	nodeProperty.SetAttributeFloat("ShadowSize", GetShadowSize());
...
//----------------------------------------------------------------------------
void Character2DFrames8::LoadFromXMLNode (XMLNode node)
{
	XMLNode nodeClass = node.GetChild("Class");
	if (!nodeClass.IsNull())
	{
		std::string typeName = nodeClass.AttributeToString("Type");
		Character::LoadFromXMLNode(nodeClass);
	}
	else
	{
		assertion(false, "");
	...
	XMLNode propertyNode = node.GetChild("Property");
	if (!propertyNode.IsNull())
	{
		SetAnimSize(propertyNode.AttributeToFloat("AnimSize"));
		APoint offset;

		if (propertyNode.HasAttribute("AnimNodeOffset_X"))
		{
			offset.X() = propertyNode.AttributeToFloat("AnimNodeOffset_X");
			offset.Y() = propertyNode.AttributeToFloat("AnimNodeOffset_Y");
			offset.Z() = propertyNode.AttributeToFloat("AnimNodeOffset_Z");
		}
		else if (propertyNode.HasAttribute("Offset_X"))
		{
			offset.X() = propertyNode.AttributeToFloat("Offset_X");
			offset.Y() = propertyNode.AttributeToFloat("Offset_Y");
			offset.Z() = propertyNode.AttributeToFloat("Offset_Z");
		}

		SetOffset(offset);

		if (propertyNode.HasAttribute("RolateNodeOffset_X"))
		{
			APoint rolateNodeOffset;
			rolateNodeOffset.X() = propertyNode.AttributeToFloat("RolateNodeOffset_X");
			rolateNodeOffset.Y() = propertyNode.AttributeToFloat("RolateNodeOffset_Y");
			rolateNodeOffset.Z() = propertyNode.AttributeToFloat("RolateNodeOffset_Z");
			SetRolateNodeOffset(rolateNodeOffset);
		}

		if (propertyNode.HasAttribute("AttackFrontOffset_X"))
		{
			APoint attackFrontOffset;
			attackFrontOffset.X() = propertyNode.AttributeToFloat("AttackFrontOffset_X");
			attackFrontOffset.Y() = propertyNode.AttributeToFloat("AttackFrontOffset_Y");
			attackFrontOffset.Z() = propertyNode.AttributeToFloat("AttackFrontOffset_Z");
			SetAttackFrontNodeOffset(attackFrontOffset);
		}

		SetShadowFilename(propertyNode.AttributeToString("ShadowFilename"));
		SetShadowSize(propertyNode.AttributeToFloat("ShadowSize"));
	}
	else
	{
		assertion(false, "");
