...
//----------------------------------------------------------------------------
void SkillStepFarTrack::SetRangeRadius (float rangeRadius)
{
	mRangeRadius = rangeRadius;
}
//----------------------------------------------------------------------------
...
void SkillStepFarTrack::OnEnter (SkillInstance *instance)
{
	...
	if (aimTargetChar)
	{
		...
		skillActor->SetRangeRadius(GetRangeRadius());
...
void SkillStepFarTrack::LoadFromXMLNode (XMLNode node)
{
	...
	if (!nodeProperty.IsNull())
	{
		...
		SetRangeRadius(nodeProperty.AttributeToFloat("RangeRadius"));
...
void SkillStepFarTrack::RegistProperties ()
{
	...
	AddProperty("RangeRadius", PT_FLOAT, GetRangeRadius());
...
void SkillStepFarTrack::OnPropertyChanged (const PropertyObject &obj)
{
	...
	else if ("RangeRadius" == obj.Name)
	{
		SetRangeRadius(PX2_ANY_AS(obj.Data, float));
	}
