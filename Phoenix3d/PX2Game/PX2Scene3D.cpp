...
void Scene3D::SetUseLightTexture (bool use)
{
	...
	for (int i=0; i<GetNumActors(); i++)
	{
		...
		else if (actor)
		{
			...
			if (mov)
			{
				_SetUseLightTexture(mov, use);
			}
		}
	}
}
//----------------------------------------------------------------------------
void Scene3D::ConvertToLightMtl ()
{
	for (int i=0; i<GetNumActors(); i++)
	{
		Actor *actor = GetActor(i);
		...
		else if (actor)
		{
			...
			if (mov)
			{
				_ConvertLightMtl(mov);
			}
		}
	}
}
//----------------------------------------------------------------------------
void Scene3D::_SetUseLightTexture (Movable *mov, bool use)
{
	if (!mov)
		return;
	...
	if (renderable)
	{
		renderable->SetUseLightTexture(use);
	}
	else if (node)
	{
		for (int i=0; i<node->GetNumChildren(); i++)
		{
			_SetUseLightTexture(node->GetChild(i), use);
		}
	}
}
//----------------------------------------------------------------------------
void Scene3D::_ConvertLightMtl (Movable *mov)
{
	if (!mov)
		return;
	...
	if (renderable)
	{
		renderable->ConvertLightMtl();
	}
	else if (node)
	{
		for (int i=0; i<node->GetNumChildren(); i++)
		{
			_ConvertLightMtl(node->GetChild(i));
		}
	}
}
//----------------------------------------------------------------------------
